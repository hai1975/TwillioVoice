    ...
    async def start_session(
        ...
        try:
            async with self.client.aio.live.connect(
                ...
                try:
                    # receive() chỉ cover 1 turn — lặp lại trên CÙNG session (Google cookbook)
                    while active():
                        turn_count += 1
                        logger.info("Gemini: chờ turn %d", turn_count)
                        try:
                            async for response in session.receive():
                                ...
                                if (
                                    ...
                                    logger.info(
                                        "Gemini: turn %d complete — chờ user nói tiếp",
                                        turn_count,
                                    )
                    ...
                    for t in (send_audio_task, send_text_task):
                        try:
                            await t
        ...
        logger.info(
            "Gemini session kết thúc (turns=%d, audio_sent=%d)",
            turn_count,
            audio_sent_count,
        )
